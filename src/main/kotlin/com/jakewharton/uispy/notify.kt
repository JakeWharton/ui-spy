...
interface ProductNotifier {
	suspend fun notify(handle: String, name: String, available: Boolean)
}
...
private val productsUrl = "https://store.ui.com/products/".toHttpUrl()
private fun handleUrl(handle: String): HttpUrl {
	return productsUrl.newBuilder().addPathSegment(handle).build()
}

private class CompositeProductNotifier(
	private val productNotifiers: List<ProductNotifier>,
) : ProductNotifier {
	override suspend fun notify(handle: String, name: String, available: Boolean) {
		for (productNotifier in productNotifiers) {
			productNotifier.notify(handle, name, available)
		}
	}
}
...
object ConsoleProductNotifier : ProductNotifier {
	override suspend fun notify(handle: String, name: String, available: Boolean) {
		if (available) {
			println("AVAILABLE: $name ${handleUrl(handle)}")
		} else {
...
class IftttProductNotifier(
	...
	override suspend fun notify(handle: String, name: String, available: Boolean) {
		val body = PostBody(
			...
			value3 = handleUrl(handle).toString(),
		)
